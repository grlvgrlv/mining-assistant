...
curl -s -o "${PROJECT_BASE}/app.py" "https://raw.githubusercontent.com/user/mining-assistant/main/app.py" || {
    echo -e "${YELLOW}Could not download app.py from GitHub. Creating it locally...${NC}"
    
    # Create app.py file with the content from our application
    cat > "${PROJECT_BASE}/app.py" << 'EOL'
#!/usr/bin/env python3
import os
import sys
import psutil
...
mkdir -p "${PROJECT_BASE}/templates"
cat > "${PROJECT_BASE}/templates/dashboard.html" << 'EOL'
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
cat > "${PROJECT_BASE}/start_system_health_dashboard.sh" << 'EOL'
#!/bin/bash
