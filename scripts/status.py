#!/usr/bin/env python3
"""
Εργαλείο ανάλυσης προόδου και κατάστασης του AI Mining Assistant

Το εργαλείο συλλέγει δεδομένα για την κατάσταση του έργου, καταγράφει την πρόοδο των διαφόρων φάσεων 
ανάπτυξης και παρέχει μια συνοπτική εικόνα της θεσμικής μνήμης και της υποδομής.

Βασικές λειτουργίες:
- Ανάλυση θεσμικής μνήμης και προσδιορισμός σημαντικών αρχείων
- Παρακολούθηση προόδου σε βασικές φάσεις ανάπτυξης (υποδομή, backend, AI, frontend, deployment)
- Έλεγχος της κατάστασης της βάσης δεδομένων PostgreSQL
- Καταγραφή εξαρτήσεων λογισμικού και επιδόσεων υπολογιστικού συστήματος
- Παρουσίαση προτεινόμενων επόμενων βημάτων
"""
import os
import json
import sys
...
class ProjectProgressTracker:
    ...
    def update_step_status(self, phase, step_name, status):
        for step in self.phases[phase]["steps"]:
            if step["name"] == step_name:
                ...
                print(f"Ενημερώθηκε το βήμα '{step_name}' σε κατάσταση '{status}'")
                break
        self.save_progress()
    ...
    def verify_postgresql_setup(self):
        ...
        try:
            ...
            print("Η επαλήθευση της PostgreSQL ολοκληρώθηκε επιτυχώς")
            return True
